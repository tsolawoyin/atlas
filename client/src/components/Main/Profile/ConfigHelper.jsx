...
// everything is getting close to perfect...
...
function ConfigHelper({
  ...
  let [topics, setTopics] = useImmer([]); // ok, I will just have to know whether the topic is selected from here or not... simple thing....
  // the topic version is very simple...
  // it just loads up the topic under a specific subject... easy peasy nothing much....
  // boots code is very simple... na me dey calculate go far...
  // everything don play for my head like madd...
  // maybe tomorrow I no go go class...
  // just be away for the mean time you get dah kind of gist... hmmmm...
  // nice one and makes sense.. nice one...
  // I should be able to handle this kind of thing now... hmmmm....
  return (
    <motion.div
      id="select-handler"
      className="p-2"
        ...
        >
          {curr
            ? topics.map((t, i) => {
                ...
                return t.selected ? ( // that's what's going to happen shey you get...
                  <motion.li
                    className="topic"
                    style={{
                      display: "grid",
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          let val = document.querySelector(
                            `#${topicName}`
                          ).value; // something like this should work
                            ...
                            if (
                              ...
                              setChoosen((draft) => {
                                let choice = draft.find(
                                  (d) =>
                                    d.subject == currSbj && d.topic == t.name
                                );
                                if (choice.qty != Number(val)) {
                                  choice.qty =
                                    Number(val) <= 60 ? Number(val) : 60; // nice one...
                                }
                              });
                            } else {
                  ...
                  // this is the normal stuff that will show up if not clicked...
                  <motion.li
                    className="subject"
                    onClick={(e) => {
